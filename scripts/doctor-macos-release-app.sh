...
APP_PATH="${1:-${ZCODE_MACOS_RELEASE_APP_PATH:-/Applications/qiyicode.app}}"
...
if [ "${APP_PATH:-}" = "--help" ] || [ "${APP_PATH:-}" = "-h" ]; then
  cat <<'USAGE'
Usage:
  bash scripts/doctor-macos-release-app.sh /Applications/qiyicode.app
  ZCODE_MACOS_RELEASE_APP_PATH=/Applications/qiyicode.app pnpm run doctor:macos-release
