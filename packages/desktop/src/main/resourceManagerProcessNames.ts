...
import { formatQiyicodeRendererProcessName } from "@zcode/shared";
...
export function buildAuxiliaryRendererName(contents: WebContents): string {
  ...
  if (url?.startsWith("devtools://")) {
    return formatQiyicodeRendererProcessName(
      `devtools-${contentsId}-${hostTitle ?? ownerLabel ?? "unknown"}`,
    );
  }

  if (type === "window") {
    return formatQiyicodeRendererProcessName(title ?? `window-${contentsId}`);
  }

  return formatQiyicodeRendererProcessName(`${type}-${contentsId}-${pageLabel}`);
}
